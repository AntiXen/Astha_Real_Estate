...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#05162E] text-[#f3f4f6] font-bengali antialiased relative">
      ...
      <main className="flex-1">
        {viewState.page === 'admin' ? (
          ...
          <div id="landing-main-view">
            <section className="relative overflow-hidden bg-[#0A1D37] text-white py-20 lg:py-28" id="hero-banner">
              ...
              <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6 lg:space-y-8">
                ...
                >
                  ...
                  <div className="md:col-span-3">
                    <a href={`tel:${settings.contactPhone}`} className="w-full h-full flex items-center justify-center space-x-1.5 rounded bg-[#C9A84C] text-[#0A1D37] hover:bg-[#b0923f] transition-all text-xs font-bold shadow cursor-pointer py-2.5 md:py-0 font-bengali">
              ...
              <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 select-none">
                  {[
                    ...
                    return (
                      <div key={item.id} className="flex items-center justify-center space-x-2.5 text-left py-1 px-2">
                        <div className="p-1.5 bg-[#C9A84C]/10 text-[#b0923f] rounded-lg shrink-0"><IconComponent className="h-4 w-4" /></div>
                        <div>
                          <div className={`text-base sm:text-lg md:text-xl font-extrabold text-[#b0923f] leading-none flex items-center ${item.isBengali ? 'font-bengali' : 'font-sans'}`}>
                            {item.isNumber ? <CountUpText target={item.target!} suffix={item.suffix} isBengali={item.isBengali} /> : <span className="relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-[#b0923f] via-[#C9A84C] to-[#dba828] animate-pulse font-extrabold font-bengali">{item.text}</span>}
                          </div>
                          <div className="text-[10px] sm:text-xs font-bold text-slate-700 font-bengali mt-0.5 leading-none">{item.title}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <section className="py-12 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6" id="category-quickboard">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {categories.map((c, idx) => {
                  ...
                  const iconList = [ <Grid key="1" className="h-5 w-5 text-[#b0923f]" />, <MapPin key="2" className="h-5 w-5 text-[#b0923f]" />, <Building key="3" className="h-5 w-5 text-[#b0923f]" />, <Sparkles key="4" className="h-5 w-5 text-[#b0923f]" /> ];
                  return (
                    <div 
                      key={c.id}
                      onClick={() => { setSelectedCategoryId(isActive ? 'all' : c.id); document.getElementById('property-showroom-section')?.scrollIntoView({ behavior: 'smooth' }); }}
                      className={`cursor-pointer bg-white p-5 rounded-lg border transition-all text-center flex flex-col items-center justify-center space-y-2 group hover:shadow-xl hover:-translate-y-0.5 ${isActive ? 'border-[#C9A84C] ring-2 ring-[#C9A84C]/45 bg-amber-50/10' : 'border-slate-250/90 hover:border-[#C9A84C]'}`}
                    >
                      <div className={`h-10 w-10 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform ${isActive ? 'bg-[#C9A84C]/20 border border-[#C9A84C]/40' : 'bg-slate-50 border border-slate-100'}`}>
            ...
            <section className="py-12 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8" id="property-showroom-section">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0A1D37] px-5 py-4 rounded-lg border border-white/10">
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setSelectedCompanyId('all')} className={`rounded-md px-3.5 py-2 text-xs sm:text-sm font-bold border transition-all cursor-pointer font-bengali ${selectedCompanyId === 'all' ? 'bg-[#C9A84C] border-[#C9A84C] text-[#0A1D37] font-extrabold shadow-sm' : 'border-white/10 text-slate-300 hover:bg-white/10 hover:text-white'}`}>
                    সমস্ত ডেভেলপার
                  </button>
                  {companies.map((comp) => {
                    const count = properties.filter((p) => p.companyId === comp.id).length;
                    return (
                      <button key={comp.id} onClick={() => setSelectedCompanyId(comp.id)} className={`rounded-md px-3.5 py-2 text-xs sm:text-sm font-bold border transition-all cursor-pointer font-bengali ${selectedCompanyId === comp.id ? 'bg-[#C9A84C] border-[#C9A84C] text-[#0A1D37] font-extrabold shadow-sm' : 'border-white/10 text-slate-300 hover:bg-white/10 hover:text-white'}`}>
                ...
                {filteredProperties.length > 0 ? (
                  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    {filteredProperties.map((prop) => (
                      <div key={prop.id}>
                        <PropertyCard property={prop} categories={categories} companies={companies} settings={settings} onSelect={(id) => handleNavigate('home', id)} />
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-14 bg-slate-50 rounded-lg border border-slate-250 max-w-lg mx-auto space-y-3 text-slate-800">
                    <div className="h-10 w-10 bg-[#C9A84C]/10 rounded-full flex items-center justify-center text-[#C9A84C] mx-auto"><Search className="h-5 w-5" /></div>
                    <div className="space-y-0.5"><p className="text-xs font-bold font-bengali">দুঃখিত! কোনো ম্যাচিং প্রজেক্ট পাওয়া যায়নি</p><p className="text-[10px] text-slate-500">ভিন্ন ডেভেলপার অথবা ভিন্ন ক্যাটাগরি ফিল্টার ট্রাই করুন।</p></div>
                    <button onClick={() => { setSearchQuery(''); setSelectedCategoryId('all'); setSelectedCompanyId('all'); }} className="rounded bg-[#C9A84C] text-[#0A1D37] px-3.5 py-2 text-[11px] font-bold hover:bg-[#b0923f] transition-all cursor-pointer font-bengali">
            ...
            <section className="py-12 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6" id="testimonials">
              <div className="text-center max-w-3xl mx-auto space-y-2">
                <span className="text-[10px] sm:text-xs font-extrabold tracking-wider text-[#C9A84C] uppercase bg-[#C9A84C]/10 px-3 py-1.5 rounded-full border border-[#C9A84C]/35 inline-block font-bengali">গ্রাহক সন্তুষ্টি আমাদের মূল লক্ষ্য</span>
                <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#C9A84C] font-bengali leading-tight">আস্থা ও ভালোবাসায় আমাদের গ্রাহক পরিবার</h3>
                <p className="text-xs sm:text-sm text-slate-300 font-bengali max-w-2xl mx-auto leading-relaxed">নিরাপদ ও ঝামেলামুক্ত উপায়ে ডিল সম্পন্ন করে যারা আস্থার ঠিকানা খুঁজে পেয়েছে, শুনুন তাদের বাস্তব অভিজ্ঞতা</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {[
                  ...
                  return (
                    <div key={item.id} onClick={() => setSelectedTestimonial(item)} className="group relative bg-[#071930] p-5 sm:p-6 rounded-2xl border border-white/10 shadow-lg hover:shadow-xl hover:border-[#C9A84C]/40 transition-all duration-300 text-left flex flex-col justify-between space-y-4 cursor-pointer transform hover:-translate-y-1">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center space-x-1">{[...Array(item.rating)].map((_, i) => <Star key={i} className="h-4 w-4 text-[#C9A84C] fill-[#C9A84C]" />)}</div>
                        <p className="text-xs sm:text-sm text-slate-200 italic leading-relaxed font-bengali font-semibold group-hover:text-white transition-colors duration-200">"{item.quote}"</p>
                        <button className="pt-1 text-[#C9A84C] text-[11px] sm:text-xs font-bold flex items-center space-x-1.5 hover:underline font-bengali"><span>বিস্তারিত কেস স্টাডি দেখুন</span><Sparkles className="h-3.5 w-3.5 shrink-0" /></button>
                      </div>
                      <div className="border-t border-white/5 pt-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="flex items-center space-x-2.5">
                          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-[#C9A84C] to-[#a38435] text-white text-xs sm:text-sm font-extrabold flex items-center justify-center font-sans shadow-md ring-2 ring-white/10">{item.initial}</div>
                          <div><div className="text-xs sm:text-sm font-bold text-white font-bengali tracking-wide">{item.name}</div><div className="text-[10px] sm:text-xs text-[#C9A84C] font-semibold font-bengali mt-0.5">{item.role}</div></div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); setTestimonialLikes(prev => ({ ...prev, [item.id]: hasLiked ? prev[item.id] - 1 : prev[item.id] + 1 })); setTestimonialUserHasLiked(prev => ({ ...prev, [item.id]: !hasLiked })); }} className={`flex items-center justify-center space-x-1.5 px-2.5 py-1.5 rounded-full border transition-all cursor-pointer ${hasLiked ? 'bg-[#C9A84C] border-[#C9A84C] text-[#0A1D37] scale-102 shadow-md' : 'bg-white/5 border-white/15 text-slate-300 hover:bg-white/10 hover:text-white hover:border-[#C9A84C]/30'}`}>
            ...
            <AnimatePresence>
              {selectedTestimonial && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md" onClick={() => setSelectedTestimonial(null)}>
                  <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 15 }} transition={{ type: "spring", duration: 0.4 }} onClick={(e) => e.stopPropagation()} className="relative bg-[#071930] w-full max-w-2xl rounded-3xl border border-white/15 p-6 md:p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
                    <button onClick={() => setSelectedTestimonial(null)} className="absolute top-4 right-4 text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 p-2 rounded-full transition-all cursor-pointer"><X className="h-5 w-5" /></button>
                    <div className="space-y-6">
                      <div className="flex items-center space-x-1">{[...Array(selectedTestimonial.rating)].map((_, i) => <Star key={i} className="h-5 w-5 text-[#C9A84C] fill-[#C9A84C]" />)}</div>
                      <div className="space-y-2 text-left"><span className="text-xs font-bold uppercase tracking-widest text-[#C9A84C] bg-[#C9A84C]/10 px-3 py-1 rounded">সাকসেস স্টোরি কেস স্টাডি</span><h4 className="text-xl sm:text-2xl font-black text-white font-bengali leading-snug mt-1">"{selectedTestimonial.quote}"</h4></div>
                      <div className="flex items-center space-x-3.5 bg-white/5 p-4 rounded-2xl border border-white/5">
                        <div className="h-12 w-12 rounded-full bg-gradient-to-br from-[#C9A84C] to-[#a38435] text-white text-base font-extrabold flex items-center justify-center font-sans tracking-tight">{selectedTestimonial.initial}</div>
                        <div className="text-left"><h5 className="text-base sm:text-md font-bold text-white font-bengali">{selectedTestimonial.name}</h5><p className="text-xs sm:text-xs text-[#C9A84C] font-semibold font-bengali">{selectedTestimonial.role}</p></div>
                      </div>
                      <div className="space-y-3.5 text-left font-bengali"><h6 className="text-[#C9A84C] text-sm sm:text-base font-bold">আস্থার ঠিকানার মাধ্যমে আইনি সত্যতা যাচাই:</h6><p className="text-sm sm:text-base text-slate-300 leading-relaxed font-light">{selectedTestimonial.story}</p></div>
                      <div className="space-y-3 pt-2 text-left font-bengali border-t border-white/10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{selectedTestimonial.verifiedFeatures.map((feat: string, idx: number) => <div key={idx} className="flex items-center space-x-2 text-xs sm:text-sm text-slate-300 font-medium"><CheckCircle2 className="h-4 sm:h-5 w-4 sm:w-5 text-emerald-400 shrink-0" /><span>{feat}</span></div>)}</div>
                      </div>
                      <div className="pt-4 flex justify-end">
                        <button onClick={() => { setSelectedTestimonial(null); setTimeout(() => { document.getElementById('lead-form')?.scrollIntoView({ behavior: 'smooth' }); }, 150); }} className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#C9A84C] text-[#0A1D37] text-sm font-bold font-bengali hover:bg-[#b0923f] transition-all cursor-pointer text-center">
              ...
              <div className="bg-slate-900 rounded-xl border border-white/10 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center shadow-2xl">
                <div className="lg:col-span-5 space-y-3 text-center lg:text-left">
                  <div className="inline-flex items-center space-x-1.5 text-xs font-bold text-[#C9A84C] uppercase tracking-widest bg-white/5 px-3 py-1.5 rounded border border-[#C9A84C]/30">
                    <Phone className="h-4 w-4" /><span>ইন্সট্যান্ট কল ব্যাক</span>
                  </div>
                  ...
                  <p className="text-[11px] text-slate-300 leading-relaxed font-bengali">আজই আমাদের কাস্টমার সাকসেস টিমের অভিজ্ঞ প্রপার্টি স্পেশালিস্টদের সাথে সরাসরি যোগাযোগ করুন। আমরা ২৪ ঘণ্টার মধ্যে ১০০% ভেরিফাইড প্রজেক্টের নথিপত্র সহ আপনাকে কল দিয়ে সাহায্য করব।</p>
                </div>
                <div className="lg:col-span-7 bg-slate-950 p-6 rounded-lg border border-white/5 shadow-inner">
                  {leadSubmitted ? (
                    <div className="text-center py-6 space-y-3">
                      <div className="h-10 w-10 text-emerald-400 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto"><ShieldCheck className="h-6 w-6" /></div>
                      <div>
                      ...
                      <button onClick={() => setLeadSubmitted(false)} className="text-[10px] text-[#C9A84C] font-semibold hover:underline font-bengali">অন্য আরেকটি রিকোয়েস্ট তৈরি করুন</button>
                    </div>
                  ) : (
                    <form onSubmit={handleLeadSubmit} className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                        <div className="space-y-1 text-left">
                          ...
                          <input required type="text" value={leadName} onChange={(e) => setLeadName(e.target.value)} disabled={isSubmittingLead} className="w-full bg-slate-900 border border-white/10 rounded px-3 py-2 text-xs text-white focus:ring-1 focus:ring-[#C9A84C] focus:outline-none disabled:opacity-50" placeholder="যেমন: এম ডি সাজিদ" />
                        </div>
                        <div className="space-y-1 text-left">
                          <label className="text-[10px] text-slate-300 font-semibold font-bengali">মোবাইল নম্বর *</label>
                          <input required type="tel" value={leadPhone} onChange={(e) => setLeadPhone(e.target.value)} disabled={isSubmittingLead} className="w-full bg-slate-900 border border-white/10 rounded px-3 py-2 text-xs text-white focus:ring-1 focus:ring-[#C9A84C] focus:outline-none disabled:opacity-50" placeholder="যেমন: ০১৭১২৩৪৫৬৭৮" />
                        </div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                        <div className="space-y-1 text-left">
                          <label className="text-[10px] text-slate-300 font-semibold font-bengali">পছন্দের এলাকা</label>
                          <input type="text" value={leadLocation} onChange={(e) => setLeadLocation(e.target.value)} disabled={isSubmittingLead} className="w-full bg-slate-900 border border-white/10 rounded px-3 py-2 text-xs text-white focus:ring-1 focus:ring-[#C9A84C] focus:outline-none disabled:opacity-50" placeholder="যেমন: ধানমন্ডি" />
                        </div>
                        <div className="space-y-1 text-left">
                          <label className="text-[10px] text-slate-300 font-semibold font-bengali">আনুমানিক বাজেট (লাখ/কোটি টাকা)</label>
                          <input type="text" value={leadBudget} onChange={(e) => setLeadBudget(e.target.value)} disabled={isSubmittingLead} className="w-full bg-slate-900 border border-white/10 rounded px-3 py-2 text-xs text-white focus:ring-1 focus:ring-[#C9A84C] focus:outline-none disabled:opacity-50" placeholder="যেমন: ৬০ লাখ টাকা" />
                        </div>
                      </div>
                      <button type="submit" disabled={isSubmittingLead} className="w-full bg-[#C9A84C] text-[#0A1D37] hover:bg-[#b0923f] transition-all py-2 rounded text-xs font-bold cursor-pointer font-bengali flex items-center justify-center disabled:opacity-75">
                        {isSubmittingLead ? 'প্রেরণ করা হচ্ছে...' : 'অনুরোধ পাঠান (Request Call Back)'}
                      </button>
                    </form>
                  )}
                </div>
            ...
            <section className="py-12 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6" id="faq-accordion">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  {[
                    { q: "প্রপার্টি কেনার জন্য কি কি ডকুমেন্ট লাগে?", a: "প্রপার্টি কেনার ক্ষেত্রে সাধারণত জাতীয় পরিচয়পত্র (NID), পাসপোর্ট সাইজ ছবি, আয়ের উৎস প্রমাণকারী ডকুমেন্ট (যেমন পে-স্লিপ বা ব্যাংক বিবরণী), এবং কর পরিশোধ সনদ (TIN) প্রয়োজন হয়।" },
                    { q: "বুকিং বা রেজিস্ট্রেশনের প্রক্রিয়া কেমন?", a: "পছন্দের প্রপার্টি বুকিংয়ের জন্য আবেদনপত্রের সাথে বায়নার টাকা (বুকিং মানি) প্রদান করতে হয়। পরবর্তীতে চুক্তিনামা সম্পাদন করে সিডিউল অনুযায়ী ডাউনপেমেন্ট ও কিস্তির টাকা শোধ করার পর রেজিস্ট্রেশন সম্পন্ন হয়।" },
                    { q: "অ্যাডভান্স দেওয়ার পর বাতিল করলে টাকা ফেরত পাবো?", a: "হ্যাঁ, বুকিং বা অ্যাডভান্স দেওয়ার পর কোনো কারণে বাতিল করতে চাইলে চুক্তিনামায় উল্লিখিত নির্দিষ্ট নিয়মাবলি ও রিফান্ড পলিসি অনুযায়ী একটি নির্দিষ্ট সময়ের মধ্যে আবেদন পেন্ডিং ফি বাদে মূল টাকা অফিশিয়ালি ফেরত পাওয়া যায়।" }
                  ].map((item, index) => {
                    const actualIndex = index;
                    const isOpen = openFaqIndex === actualIndex;
                    return (
                      <div key={actualIndex} className="bg-[#0A1D37]/45 border border-white/10 rounded overflow-hidden transition-all hover:border-[#C9A84C]/30">
                        <button onClick={() => setOpenFaqIndex(isOpen ? null : actualIndex)} className="w-full flex items-center justify-between p-4 text-left font-bold text-xs text-white hover:text-[#C9A84C] transition-colors">
                          <span className="font-bengali text-left">{item.q}</span><ChevronDown className={"h-4 w-4 shrink-0 text-[#C9A84C] transition-transform " + (isOpen ? "rotate-180" : "")} />
                        </button>
                        {isOpen && <div className="p-4 pt-0 text-[11px] text-slate-300 leading-relaxed border-t border-white/5 font-bengali">{item.a}</div>}
                      </div>
                    );
                  })}
                </div>
      ...
      {viewState.page !== 'admin' && <Footer settings={settings} onNavigate={(view) => handleNavigate(view)} />}

      {viewState.page !== 'admin' && (
        <div className="fixed bottom-6 right-6 z-40 flex flex-col space-y-3" id="global-floating-ctas">
          <a href={`tel:${settings.contactPhone}`} className="flex h-13 w-13 items-center justify-center rounded-full bg-slate-800 text-brand-gold-500 hover:text-white shadow-xl hover:shadow-2xl hover:scale-105 transition-all text-center border-2 border-brand-gold-500/20" title="আমাদের সরাসরি ফোন করুন" id="floating-phone-btn">
            <Phone className="h-6 w-6 fill-brand-gold-500/10" />
          </a>
          <a href={globalWhatsappUrl} target="_blank" rel="noreferrer" className="flex h-13 w-13 items-center justify-center rounded-full bg-emerald-600 text-white shadow-xl hover:shadow-2xl hover:bg-emerald-700 hover:scale-105 transition-all border-2 border-emerald-500/20 active:scale-95 animate-bounce-custom" title="হোয়াটসঅ্যাপে ফ্রি কনসালটেন্ট সাপোর্ট পান" id="floating-whatsapp-btn">
            <MessageSquare className="h-6 w-6 fill-white text-emerald-600" />
          </a>
        </div>
      )}

      {viewState.propertyId && <PropertyDetailsModal propertyId={viewState.propertyId} properties={properties} categories={categories} companies={companies} settings={settings} onClose={() => handleNavigate('home', null)} />}
    </div>
  );
}