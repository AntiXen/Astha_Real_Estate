...
export interface LocalAdminUser {
  ...
  name: string;
  email: string;
  password: string; // Stored securely as SHA-256 hash
  isSuper?: boolean;
}
...
// 🔒 Cryptographic password hashing helper for browser-based offline demo fallback
...
export async function signInAdmin(email: string, password: string) {
  ...
  // 🔒 Offline Mode: Compare securely against hashed passwords
  const admins = readLocalAdmins();
  const inputHash = await hashPassword(password);
  const found = admins.find(
    (admin) => admin.email.toLowerCase() === email.toLowerCase() && admin.password === inputHash
  );
  if (!found) {
...
export async function signUpAdmin(name: string, email: string, password: string) {
  if (isSupabaseEnabled && supabase) {
    ...
    // Securely invoke serverless Edge Function to register user
    ...
    if (wasBootstrap && data?.user) {
      // Bootstrap case: nobody was logged in yet, so we sign them in locally
  ...
  // 🔒 Offline Mode: Cryptographically hash fallback password before saving
  ...
  return { user: { userId: newAdmin.id, name: newAdmin.email, email: newAdmin.email, isSuper: makeSuperLocal } };
}
...
export async function deleteAdminUser(email: string) {
  if (isSupabaseEnabled && supabase) {
    ...
    // Invoke serverless Edge Function to delete sub-admin
